...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchVisitorStats = async () => {
            try {
                const response = await fetch('https://p-server-mu.vercel.app/api/visitor-stats');
            ...
            <ul className="bg-gray-100 rounded-lg p-4 space-y-2">
                {data.visitors.map(visitor => (
                    <li key={visitor._id} className="p-2 bg-white rounded-lg shadow">
                        ...
                        <p className="font-medium text-cyan-800"><span className="font-bold">IP: </span> {visitor.ip}</p>
                            <p className="text-gray-700"><span className="font-bold">Location:</span> {visitor.location}</p>
                            <p className="text-gray-700"><span className="font-bold">User Agent:</span> {visitor.userAgent}</p>
                        </div>
