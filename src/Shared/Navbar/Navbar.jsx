...
const Navbar = () => {
  const PASSWORD = "galib2723";
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const navigate = useNavigate();
  const promptPass = async () => {
  ...
  const handleClick = async () => {
    await promptPass();
    if (isAuthenticated) {
      navigate("/galib/dashboard-stats");
    } else {
      return;
    }
  };
  ...
  const navLinks = (
    <>
      ...
      <li className="active:border-b-2">
        <Link to="design" spy={true} smooth={true} duration={500}>
          DESIGNS
        </Link>
      </li>
      <li className="active:border-b-2">
        <Link to="featured" spy={true} smooth={true} duration={500}>
          FEATURED
        </Link>
      </li>
      <li className="active:border-b-2">
    ...
    <div className="navbar w-full bg-gray-500 bg-opacity-40 md:fixed md:z-30 text-cyan-300">
      <div className="navbar-start">
        ...
        <div className="flex items-center">
          ...
          <h2 className="hidden md:flex text-lg font-semibold">
            ASADULLAH AL GALIB
          </h2>
        </div>
      </div>
      ...
      <div className="navbar-end">
        <details className="dropdown dropdown-left dropdown-bottom z-50">
          <summary className="btn btn-accent btn-outline border-cyan-300 border-2">
            <FiAlignJustify className="text-xl text-cyan-300"></FiAlignJustify>
          </summary>
          <ul
            className="p-2 shadow menu dropdown-content z-[1] rounded-box w-60 bg-blue-800 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-20 border border-gray-100
"
          >
